...
def run(t,res,plant):
    ...
    baresoil(c.Kr(),0.,c.get_Rn(t, 0.12, True),c.get_tmean(t),c.get_es(t),c.get_ea(t), c.get_windspeed(t), 0.12,0.12,100.,RHmin=30.,h=1.)
    ...
    res.supply.append(plant.supply)if plant else res.supply.append(0)
...
class Res(object):
    def __init__(self):
        ...
        self.alpha = []
    def __repr__(self):
        return "Shoot=%gg, Root=%gg, ETc = %gmm, Wateruptake=%gmm, Stress=%g" % (self.shoot_biomass[-1],self.root_biomass[-1],self.ETc[-1],sum(self.water_uptake[-1]),self.supply[-1])
...
if __name__=='__main__':
    ...
    showit(res.matrix_potential,4,5,cmap=cm.RdYlBu,vmin=-1.6,vmax=0)
    showit(res.wetness,5,5,cmap=cm.RdYlBu,vmax=1.,vmin=0.)
    show()