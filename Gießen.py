...
class Crop:
    def __init__(self,soil,atmosphere,param_file):
        self.param_file = param_file
        
        self.stage=[['Emergence',160.],['Leaf development',208.],['Tillering',421.],['Stem elongation',659.],
                       ['Anthesis',901.],['Seed fill',1174.],['Dough stage',1556.],['Maturity',1665.]]
        
        #Partitioning coefficiants for each season:
        self.shoot_percent =[.0,.9,.9,.9,.95,1.,1.,1.]
        self.root_percent = [.0,.1,.1,.1,.05,.0,.0,.0]
        self.leaf_percent = [[.0,.5,.5,.3,0.,.0,.0,.0][i]*perc for i,perc in enumerate(shoot_percent)]
        self.stem_percent = [[.0,.5,.5,.7,.3,.0,.0,.0][i]*perc for i,perc in enumerate(shoot_percent)]
        self.storage_percent = [[.0,.0,.0,.0,.7,1.,1.,1.][i]*perc for i,perc in enumerate(shoot_percent)]
        
        #Evapotranspiration crop parameter
        #Length of initil-, crop development-, mid- and end season
        self.seasons = [stage[0][1], stage[3][1]-stage[0][1], stage[6][1]-stage[3][1], stage[-1][1]-stage[3][1]]
        #basal crop coefficiants for each seasnon
        self.Kcb_values = [0.15,1.1,0.15]
        ...
        self.tbase=0.
        self.rootability_thresholds=[1.5,0.5,16000.,.0,0.0,0.0]
        self.pressure_threshold=[0.,1.,500.,16000.]
        self.plant_N=[[160.,0.43],[1174.,0.16]]
        self.leaf_specific_weight=50.
        self.root_growth=1.2
        self.K_m=0.
        self.c_min=0.
        params = [stage,shoot_percent,root_percent,leaf_percent,stem_percent,storage_percent,seasons,Kcb_values,LUE,k,tbase,rootability_thresholds,
                  pressure_threshold,plant_N,leaf_specific_weight,root_growth,K_m,c_min]
        
        #Parameter development:
        #Name growth stage, totat thermaltime of ending
        self.summerwheat =  FlowerPower.Plant(soil,atmosphere,ET_FAO(self.Kcb_values,self.seasons),Water_Feddes(),
                                         Biomass_LUE(self.LUE,self.k),Development(self.stage),SoilLayer(),
                                        Nitrogen(),self.shoot_percent,self.root_percent,self.leaf_percent,self.stem_percent,self.storage_percent,
                                        self.tbase,self.rootability_thresholds,self.pressure_threshold,self.plant_N,
                                        self.leaf_specific_weight,self.root_growth) 
        self.barely = FlowerPower.Plant
        self.oat = FlowerPower.Plant
    @property
    def SummerWheat(self):
        return self.sommerwheat
    @property
    def Barley(self):
        return self.barley
    def Oat(self):
        return self.oat
    def crop_params(self,param_file,crop = 'SummerWheat'):
        return [line for line in open(param_file)]
