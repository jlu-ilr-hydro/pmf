...
def run(time_act,time_period,time_step,sowing):
    run_end=time_act+time_period
    while time_act<=run_end:
        if time_act >= sowing:
            plant(time_act,time_step.days,1000,0.05,0.5,0.3,0.7,5.0,development,soil,atmosphere)
        sim.result(Plant_drymass, plant.Wp_total)
        sim.result(Plant_thermaltime,plant.stage.total_thermaltime)
        sim.result(Atmosphere_tmin,atmosphere.get_tmax(time_act))
        sim.result(Root_depth, plant.root.depth)
        time_act+=time_step
       
       

    
run(datetime(2009,1,1),timedelta(365),timedelta(1),datetime(2009,4,1))
sim.graph(1)
